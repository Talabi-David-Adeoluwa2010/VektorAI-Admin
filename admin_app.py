...
if users_data:
    ...
    st.dataframe(df[['username', 'status', 'payment_status', 'registered_at', 'last_active', 'license_expiry']], use_container_width=True)
else:
...
with col1:
    days_to_add = st.number_input("Days of Access to Grant:", min_value=1, value=30)
    ...
    if st.button("Generate New Activation PIN", type="primary"):
        ...
        pins_db[new_pin] = {
            "status": "Unused",
            "created_at": datetime.now(ZoneInfo('Africa/Lagos')).strftime("%Y-%m-%d %H:%M:%S"),
